...
def train_with_wandb(
    ...
    with wandb.init(**wandb_init_args) as run:
        ...
        output_dir = os.path.join("models", project_name, f"{experiment_name}_{config['version']}")
...
def main():
    ...
    model.compile(
        ...
        loss={key: settings['ft']['loss'] for key in ['family', 'genus', 'species']},
        metrics={key: settings['ft']['metrics'] for key in ['family', 'genus', 'species']},
        loss_weights={
            'family': settings['ft']['loss_weights'][0],
            'genus': settings['ft']['loss_weights'][1],
            'species': settings['ft']['loss_weights'][2],
        },
    )

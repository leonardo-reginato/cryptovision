...
def main(
    ...
    def residual_block(x, filters, kernel_size=3, stride=1):
        ...
        if shortcut.shape[-1] != filters:
            # If input and output channels are different, adjust the shortcut
    ...
    # Now modify the architecture after the base model
    for model_name in pre_trained_models.keys():
        
        MODEL_PARAMS['base_model'] = model_name
        MODEL_PARAMS['resnet_arch'] = True
        ...
        x = preprocess_input(x)  # Preprocessing input for the pre-trained model
        x = base_model(x, training=False)  # Pre-trained model

        # Apply residual blocks after the base model
        x = residual_block(x, filters=256)  # First residual block
        x = residual_block(x, filters=512)  # Second residual block

        if config.attention_layer:
            # Apply custom attention layer if required
            x = AttentionLayer()(x)

...
class CryptoVisionModels:
    ...
    def augmentation_layer(
        seed: int = 42,
        zoom_factor: tuple[int] = (0.05, 0.1),
        contrast: float = 0.2,
        ...
        aug_layer = tf.keras.Sequential(
            [
                ...
                layers.RandomZoom(
                    height_factor=zoom_factor, width_factor=zoom_factor, seed=seed
                ),  # Wider zoom range
    ...
    def backbone_and_preprocess(pretrain: str, image_shape=(224, 224, 3)):
        model_map = {
            ...
            "efv2b0": (
                keras_apps.EfficientNetV2B0,
                keras_apps.efficientnet_v2.preprocess_input,
            ),
            "efv2b1": (
                keras_apps.EfficientNetV2B1,
                keras_apps.efficientnet_v2.preprocess_input,
            ),
            "vgg16": (keras_apps.VGG16, keras_apps.vgg16.preprocess_input),
        }
        model = model_map[pretrain][0](
    ...
    def build_feature_extractor(
        backbone, preprocess, input_shape, name=None, augmentation=None
    ):
    ...
    @staticmethod
    def dense_block(
        input_layer, name, units, dropout=0.3, activation="relu", norm=True, **kwargs
    ):
        """
    ...
    @staticmethod
    def taxonomy_conditioned_attention(
        shared_features, conditioning_logits, attention_units=128, name_prefix="attn"
    ):
        """
        ...
        """
        x = layers.Dense(
            attention_units, activation="relu", name=f"{name_prefix}_cond_proj1"
        )(conditioning_logits)
        x = layers.Dense(
            shared_features.shape[-1],
            activation="sigmoid",
            name=f"{name_prefix}_cond_proj2",
        )(x)
        modulated = layers.Multiply(name=f"{name_prefix}_modulate")(
            [shared_features, x]
        )
        return modulated

    @staticmethod
    def gated_hierarchical_fusion(
        shared_features, conditioning_logits, name_prefix="gated_fusion"
    ):
        """
        ...
        cond_proj = layers.Dense(
            shared_dim, activation="relu", name=f"{name_prefix}_cond_proj"
        )(conditioning_logits)

        # Compute gate (how much to attend to conditioning info)
        gate = layers.Dense(
            shared_dim, activation="sigmoid", name=f"{name_prefix}_gate"
        )(conditioning_logits)
        ...
        conditioned_mod = layers.Multiply(name=f"{name_prefix}_cond_path")(
            [cond_proj, gate]
        )
        shared_mod = layers.Multiply(name=f"{name_prefix}_shared_path")(
            [shared_features, gate_inv]
        )
    ...
    def basic(
        ...
        input_shape: tuple[int] = (224, 224, 224),
        shared_dropout: float = 0.3,
        feat_dropout: float = 0.3,
        pooling_type: str = "max",
        architecture: str = "std",
        name: str = None,
        augmentation=None,
        ...
        imagenet_model, preprocess = CryptoVisionModels.backbone_and_preprocess(
            imagenet_name, input_shape
        )
        ...
        if se_block:
            feat_map = CryptoVisionModels.se_block(
                feat_map, reduction_ratio=16, name_prefix="se_backbone"
            )
        ...
        if pooling_type == "max":
            ...
            raise ValueError(
                f"Invalid pooling type: {pooling_type}. Choose 'max' or 'avg'."
            )
        ...
        if feat_dropout and feat_dropout > 0:
            ...
            features = layers.Dropout(
                feat_dropout, name=f"features_DropOut_{feat_dropout}"
            )(features)
        ...
        shared_layer = CryptoVisionModels.dense_block(
            features,
            "shared_layer",
            shared_layer_neurons or features.shape[-1],
            shared_dropout,
        )

        # Family
        family_output = layers.Dense(
            output_neurons[0], activation="softmax", name="family"
        )(shared_layer)

        #  Genus
        if architecture == "concat":
            genus_input = layers.Concatenate(name="genus_input")(
                [shared_layer, family_output]
            )
        ...
        genus_output = layers.Dense(
            output_neurons[1], activation="softmax", name="genus"
        )(genus_input)

        # Species
        if architecture == "concat":
            species_input = layers.Concatenate(name="species_input")(
                [shared_layer, genus_output]
            )
        elif architecture == "att":
        ...
        species_output = layers.Dense(
            output_neurons[2], activation="softmax", name="species"
        )(species_input)
    ...
    def se_block(input_tensor, reduction_ratio=16, name_prefix="se"):
        ...
        se = layers.Dense(
            units=channels, activation="sigmoid", name=f"{name_prefix}_expand"
        )(se)

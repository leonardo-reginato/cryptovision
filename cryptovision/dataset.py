...
class DatasetLoader:
    def __init__(self, sources: Dict[str, str], min_samples: int = 10, stratify_col: str = 'species', verbose: bool = True):
        """
    ...
    def load_catalog(self, dir_path: str) -> pd.DataFrame:
        ...
        if self.verbose:
            logger.info(f"Loaded catalog from {catalog_path} with {df.shape[0]} records.")
        return df
    ...
    def apply_quality_control(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'hash' in data.columns:
            ...
            if self.verbose:
                logger.info(f"Removed {initial_count - data.shape[0]} duplicates based on 'hash'.")
        ...
        if self.stratify_col in data.columns:
            ...
            if self.verbose:
                ...
                logger.info(f"Filtered out {removed} records with insufficient samples in '{self.stratify_col}'.")
            data = filtered
        ...
        if 'flag_small' in data.columns:
            ...
            if self.verbose:
                logger.info(f"Removed {initial_count - data.shape[0]} records flagged as small images.")
        
        if self.verbose:
            logger.info(f"Dataset size after quality control: {data.shape[0]}")
        return data

    def split_data(self, data: pd.DataFrame, test_size: float = 0.2, val_size: float = 0.2, random_state: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        """
        stratify_series = data[self.stratify_col] if self.stratify_col in data.columns else None
        train_val, test = train_test_split(data, test_size=test_size, stratify=stratify_series, random_state=random_state)
        
        # Adjust validation size relative to the remaining data after removing the test set.
        val_ratio = val_size / (1 - test_size)
        stratify_series = train_val[self.stratify_col] if self.stratify_col in train_val.columns else None
        train, val = train_test_split(train_val, test_size=val_ratio, stratify=stratify_series, random_state=random_state)
        
        if self.verbose:
            logger.info(f"Split data into train ({train.shape[0]}), validation ({val.shape[0]}), and test ({test.shape[0]}) records.")
        return train, val, test

def load_dataset(src_path: str, min_samples: int = 10, verbose: bool = True, return_split: bool = False, 
                 test_size: float = 0.2, val_size: float = 0.2, stratify_by: str = 'species', random_state: int = 42):
    """
    ...
    loader = DatasetLoader(sources=default_sources, min_samples=min_samples, stratify_col=stratify_by, verbose=verbose)
    data = loader.load_data()
    data = loader.apply_quality_control(data)
    
    if return_split:
        return loader.split_data(data, test_size=test_size, val_size=val_size, random_state=random_state)
    else:
        return data
...
if __name__ == "__main__":
    ...
    def cli(
        ...
        result = load_dataset(src_path, min_samples, True, return_split, test_size, val_size, stratify_by, random_state)
        if return_split:
            train, val, test = result
            logger.info(f"Train size: {train.shape}, Validation size: {val.shape}, Test size: {test.shape}")
        else:
